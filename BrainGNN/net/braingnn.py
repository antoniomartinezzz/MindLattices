...
from BrainGNN.net.braingraphconv import MyNNConv



##########################################################################################################################
class Network(torch.nn.Module): #cambiar R a numero de ROIS (200 para CC y 111 para HO)
    def __init__(self, indim, ratio, nclass, R, k=8):
        '''
    ...
    def forward(self, x, edge_index, batch, edge_attr, pos):
        ...
        x = F.dropout(x, p=0.5, training=self.training) #cambio de 0.5 a 0.3??
