...
parser.add_argument('--batchSize', type=int, default=32, help='size of the batches') #cambio de 100 a 32
parser.add_argument('--dataroot', type=str, default='/media/SSD2/MindGraphs/data/ABIDE_pcp/cpac/filt_noglobal', help='root directory of the dataset')
parser.add_argument('--fold', type=int, default=0, help='training which fold')
parser.add_argument('--lr', type = float, default=0.001, help='learning rate') #cambio de 0.01 a 0.001 
parser.add_argument('--stepsize', type=int, default=20, help='scheduler step size')
parser.add_argument('--gamma', type=float, default=0.5, help='scheduler shrinking rate')
parser.add_argument('--weightdecay', type=float, default=5e-3, help='regularization')
parser.add_argument('--lamb0', type=float, default=1, help='classification loss weight')
parser.add_argument('--lamb1', type=float, default=0, help='s1 unit regularization') #cambio de 0 a 0.1
parser.add_argument('--lamb2', type=float, default=0, help='s2 unit regularization') #cambio de 0 a 0.1 
...
wandb.init(project="baseline_final", entity="mind-graphs", name=opt.atlas+opt.fold)
wandb.config = {"n_epochs": opt.n_epochs, "batchSize": opt.batchSize, "fold": opt.fold, "lr": opt.lr, 
                "stepsize": opt.stepsize, "gamma": opt.gamma, "weightdecay": opt.weightdecay}
...
tr_index, te_index = train_val_test_split(fold=fold)
train_dataset = dataset[list(tr_index)] #cambia por list(...) por error IndexError: Only integers, slices (`:`), list, tuples, and long or bool tensors are valid indices (got ndarray).
...
#se elimina par HO (no usa scheduler) ??
...
def train(epoch):
    print('train...........')
    scheduler.step() #se elimina para HO??
...
if not opt.load_model:
    for epoch in range(0, num_epoch):
        ...
        tr_acc, _, _, _, _, _, _ = test_acc(train_loader)
        ...
        wandb.log({"Epoch": epoch, "Loss": tr_loss, "Fold": fold, "Acc": tr_acc})
        
        #if tr_loss < best_loss and epoch > 5:
        if epoch == (num_epoch-1):
            print("saving best model")
            best_loss = tr_loss
            best_model_wts = copy.deepcopy(model.state_dict())
            if save_model:
                torch.save(best_model_wts, os.path.join(opt.save_path,str(fold)+opt.atlas+'.pth'))
...
if opt.load_model:
    ...
    model.eval()
    test_accuracy, test_sensitivity, test_specificity, test_precision, test_recall, preds, trues = test_acc(test_loader)
    ...
    print("===========================")
    print("Test Acc: {:.7f}, Test Sensitivity:  {:.7f}, Test Specificity:  {:.7f}, Test Precision: {:.7f}, Test Recall: {:.7f}, Test Loss: {:.7f} ".format(test_accuracy, test_sensitivity, test_specificity, test_precision, test_recall, test_l))
    print(opt)

else:
    model.load_state_dict(best_model_wts)
    model.eval()
    test_accuracy, test_sensitivity, test_specificity, test_precision, test_recall, _, _ = test_acc(test_loader)
    test_l= test_loss(test_loader,0)
    print("===========================")
    print("Test Acc: {:.7f}, Test Sensitivity:  {:.7f}, Test Specificity:  {:.7f}, Test Precision: {:.7f}, Test Recall: {:.7f}, Test Loss: {:.7f} ".format(test_accuracy, test_sensitivity, test_specificity, test_precision, test_recall, test_l))
    wandb.log({"Test_Loss": test_loss, "Test_Acc": test_accuracy, "Test_Sen": test_sensitivity, "Test_Spe": test_specificity, "Test_Pre": test_precision, "Test_Rec": test_recall, "OPT": opt})
    print(opt)
